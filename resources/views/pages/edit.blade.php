...
<div class="container mt-4">
    <h1 class="mb-4">Sayfa Düzenle</h1>
    ...
    <form action="{{ route('pages.update', $page) }}" method="POST">
            ...
            <select class="form-select @error('profile_id') is-invalid @enderror"
                ...
                @foreach($profiles as $profile)
                    <option value="{{ $profile->id }}"
                        {{ old('profile_id', $page->profile_id) == $profile->id ? 'selected' : '' }}>
                        {{ $profile->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('page_id') is-invalid @enderror"
                   id="page_id" name="page_id" value="{{ old('page_id', $page->page_id) }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('access_token') is-invalid @enderror"
                   id="access_token" name="access_token" value="{{ old('access_token', $page->access_token) }}">
